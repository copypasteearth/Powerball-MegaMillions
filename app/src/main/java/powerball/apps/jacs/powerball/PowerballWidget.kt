 ...
 */
class PowerballWidget : AppWidgetProvider() {
...
internal fun updateAppWidget1(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
    ...
    GlobalScope.launch {
        withContext(Dispatchers.IO){
            ...
            val gson = Gson()
            val megamillionsData = gson.fromJson(json, PowerballData::class.java)
            ...
            var onewin = false
            val myPlays = SharedPrefHelper.getMyTickets(context, Constants.POWER_TICKETS)
                ...
                for (j in split.indices) {
                    ...
                    if (j == split.size - 1) {
                        if (split[j].toInt() == winner[j].toInt()) {
                            ...
                            log.setInt(images[j], "setBackgroundResource", R.drawable.power1)
                            log.setTextColor(images[j], Color.BLACK)
                            log.setTextViewText(images[j], split[j])
                        }
                    } else {
